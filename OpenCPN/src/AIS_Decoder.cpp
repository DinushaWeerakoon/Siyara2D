...
AIS_Error AIS_Decoder::Decode( const wxString& str )
{
    ...
    //  OK, looks like the sentence is OK
        ...
        if( mmsi || ( !string_to_parse.IsEmpty() && ( string_to_parse.Len() < AIS_MAX_MESSAGE_LEN ) ) ) {
            ...
            {
                AWship1Lat = pTargetData->Lat;
                AWship1Lon = pTargetData->Lon;
            }
            else if (pTargetData->MMSI == AWmmsi2)
            {
                AWship2Lat = pTargetData->Lat;
                AWship2Lon = pTargetData->Lon;
            }
            //-anchorwatch---------------------
...
/*
 * Created by Randika Ariyarathna
 * Modified by Chamal Perera
 */
void AIS_Decoder::VTMCoordinator_Data()
{
    wxString VTMCoordinator_Query = _T("select name,in_charge,start_date_time from coordinator_details where status = 1 order by in_charge desc");
    
    if (mysql_query(connection, (std::string(VTMCoordinator_Query.mb_str())).c_str()))
    {
        return;
        
        //wxLogMessage(_T("Error in executing MySQL query"));
        
        /*wxString msg(VTMCoordinator_Query);
         msg.Prepend(_("Failed to execute : "));
    ...
    else
    {
        ...
        {
            MYSQL_ROW row;
            int row_count = mysql_num_rows(res_set);
            int counter = 0;
            while ((row = mysql_fetch_row(res_set)) != NULL)
            {
                ...
                char name[30];
                strcpy(name, row[0]);
                VTMCoordinators.Append(trimAISField(name));
                
                int incharge = atoi(row[1]);
                if (incharge)
                    VTMCoordinators.append(_T(" (Incharge)"));
                
                wxString datetime_from_db(row[2], wxConvUTF8);
                
                wxStringTokenizer tkz(datetime_from_db, wxT(" "));
                wxString duty_on_date_stamp = tkz.GetNextToken();
                wxString duty_on_time_stamp = tkz.GetNextToken();
                
                wxDateTime tst;
                wxDateTime formated_time = wxDateTime::Now();
                
                if (tst.ParseDate(duty_on_date_stamp) && tst.ParseTime(duty_on_time_stamp))
                {
                    formated_time.SetYear(tst.GetYear());
                    formated_time.SetMonth(tst.GetMonth());
                    formated_time.SetDay(tst.GetDay());
                    
                    formated_time.SetHour(tst.GetHour());
                    formated_time.SetMinute(tst.GetMinute());
                    formated_time.SetSecond(tst.GetSecond());
                }
                
                wxDateTime now_time = wxDateTime::Now();
                
                int working_time_seconds = now_time.GetTicks() - formated_time.GetTicks();
                
                int worked_hours = working_time_seconds / 3600;
                int worked_minutes = (working_time_seconds % 3600) / 60;
                //int worked_seconds = (working_time_seconds % 3600) % 60;
                
                wxString hour_string, minutes_string, seconds_string;
                hour_string << worked_hours;
                minutes_string << worked_minutes;
                //seconds_string << worked_seconds;
                
                VTMCoordinators.append(_T(" "));
                VTMCoordinators.append(hour_string);
                VTMCoordinators.append(_T("H. "));
                VTMCoordinators.append(minutes_string);
                if (counter == row_count)
                    VTMCoordinators.append(_T("M."));
                else
                    VTMCoordinators.append(_T("M.\n"));
                //VTMCoordinators.append(seconds_string);
                //VTMCoordinators.append(_T("S.\n"));
            }
        }
        else
            VTMCoordinators = _T("New Pilot Station");
    }
}
//-showpilotstationdata------------------
...
void AIS_Decoder::MySQL_DB_User()
{
    wxMessageDialog *md;
    if (AIS_Query.Contains(_("lDT")))
    {
        ...
        //{
        //	checkHistory = true;
        //	checkHistoryDone = false;
        //	wxString msg(_("Please wait until ships appear on map."));
        //	md = new wxMessageDialog(cc1, msg, _("Siyara Message"), wxICON_INFORMATION | wxCENTRE);
        //	md->ShowModal();
        //	cc1->Disable();
        //	//md->istop
        ...
        //	//md->Disable();
        //	//bool dis=md->Disable();
        //	/*std::string tempS = BoolToString(dis);
        //	wxString mystring(tempS.c_str(), wxConvUTF8);
    ...
    //wxLogMessage(AIS_Query);
    if (mysql_query(connection, (std::string(AIS_Query.mb_str())).c_str()))
    {
        return;
        
        //wxLogMessage(_T("Error in executing MySQL query"));
        
        /*wxString msg(AIS_Query);
         msg.Prepend(_("Failed to execute : "));
         OCPNMessageDialog md(NULL, msg, _("OpenCPN Message"), wxICON_ERROR );
         md.ShowModal();*/
    }
    else
    {
        ...
        if (mysql_num_rows(res_set) != 0) // check for non-empty data set for a valid query
        {
            have_updated_data = true;
            
            MYSQL_ROW row;
            while ((row = mysql_fetch_row(res_set)) != NULL)
            {
                if (Previous_AIS_Query != AIS_Query)
                {
                    Previous_AIS_Query = AIS_Query;
                ...
                int mmsi = 0;
                if (row[0] != NULL)//newly added
                {
                    mmsi = atoi(row[0]); //MMSI from DB
                }
                long mmsi_long = mmsi;
                ...
                if (row[1] != NULL)
                    pTargetData->NavStatus = atoi(row[1]);	// NavStatus from DB
                
                if (row[2] != NULL)
                    pTargetData->ROTAIS = atoi(row[2]);	// ROTAIS from DB
                
                double rot_dir = 1.0;
                
                if (pTargetData->ROTAIS == 128)
                    pTargetData->ROTAIS = -128;                     // not available codes as -128
                else if ((pTargetData->ROTAIS & 0x80) == 0x80)
                {
                    pTargetData->ROTAIS = pTargetData->ROTAIS - 256;       // convert to twos complement
                    rot_dir = -1.0;
                }
                pTargetData->ROTIND = wxRound(rot_dir * pow((((double)pTargetData->ROTAIS) / 4.733), 2));      // Convert to indicated ROT
                
                wxThread::Sleep(40); ///-------------------------------2
                
                if (row[3] != NULL)
                    pTargetData->SOG = atof(row[3]);	// SOG from DB
                
                if (row[4] != NULL)
                    pTargetData->Lon = atof(row[4]);    // Lon from DB
                
                if (row[5] != NULL)
                    pTargetData->Lat = atof(row[5]);	// Lat from DB
                ...
                if (row[6] != NULL)
                    pTargetData->COG = atoi(row[6]);	// COG from DB
                
                if (row[7] != NULL)
                    pTargetData->HDG = atoi(row[7]);	// HDG from DB
                
                wxString time_from_db(row[9], wxConvUTF8); // last_received_time from DB
                wxStringTokenizer tkz(time_from_db, wxT(":"));
                
                wxString db_hours = tkz.GetNextToken();
                wxString db_minutes = tkz.GetNextToken();
                wxString db_seconds = tkz.GetNextToken();
                
                pTargetData->m_utc_sec = wxAtoi(db_seconds);
                
                pTargetData->db_reported_time = db_hours;
                pTargetData->db_reported_time.append(_T(":"));
                pTargetData->db_reported_time.append(db_minutes);
                pTargetData->db_reported_time.append(_T(":"));
                pTargetData->db_reported_time.append(db_seconds);
                
                wxThread::Sleep(40); ///-------------------------------3
                
                if (row[10] != NULL)
                    pTargetData->SyncState = atoi(row[10]);	// SyncState from DB
                
                if (row[11] != NULL)
                    pTargetData->SlotTO = atoi(row[11]);	// SlotTO from DB
                
                if ((pTargetData->SlotTO == 1) && (pTargetData->SyncState == 0))
                {
                    pTargetData->m_utc_min = wxAtoi(db_minutes);
                    pTargetData->m_utc_hour = wxAtoi(db_hours);
                }
                
                pTargetData->blue_paddle = 0;
                pTargetData->b_blue_paddle = false;
                
                pTargetData->Class = AIS_CLASS_A;
                int mmsi_start = pTargetData->MMSI / 10000000;
                ...
                if (mmsi_start == 99)
                {
                    ...
                    pTargetData->IMO = 0;
                    pTargetData->ROTAIS = -128;
                    if (row[15] != NULL)
                        strcpy(pTargetData->ShipName, row[15]);
                    pTargetData->b_nameValid = true;
                    
                    if (row[16] != NULL)
                        pTargetData->ShipType = (unsigned char)atoi(row[16]);
                    
                    if (row[17] != NULL)
                        pTargetData->DimA = atoi(row[17]);
                    
                    if (row[18] != NULL)
                        pTargetData->DimB = atoi(row[18]);
                    
                    if (row[19] != NULL)
                        pTargetData->DimC = atoi(row[19]);
                    
                    if (row[20] != NULL)
                        pTargetData->DimD = atoi(row[20]);
                    
                    pTargetData->Draft = 0;
                }
                else if (mmsi_start == 97)
                {
                    pTargetData->Class = AIS_SART;
                    pTargetData->StaticReportTicks = now.GetTicks();
                    pTargetData->m_ack_time = wxDateTime::Now() - wxTimeSpan::Day();
                }
                ...
                if (row[12] != NULL) // AIS version from DB
                {
                    int AIS_version_indicator = atoi(row[12]);
                    
                    if (AIS_version_indicator < 2)
                    {
                        if (row[13] != NULL)
                            pTargetData->IMO = atoi(row[13]);
                        
                        if (row[14] != NULL)
                            strcpy(pTargetData->CallSign, row[14]);
                        ...
                        if (row[22] != NULL)
                            pTargetData->Draft = atof(row[22]) / 10.0;
                        
                        if (row[23] != NULL)
                            strcpy(pTargetData->Destination, row[23]);
                ...
                //    Compute the current Range/Brg to the target
                double brg, dist;
                DistanceBearingMercator(pTargetData->Lat, pTargetData->Lon, gLat, gLon, &brg, &dist);
                pTargetData->Range_NM = dist;
                pTargetData->Brg = brg;
                
                if (dist <= 1e-5)
                    pTargetData->Brg = -1.0;             // Brg is undefined if Range == 0.
                ...
                pTargetData->PositionReportTicks = now.GetTicks();//Newly added
                //  Update the most recent report period
                pTargetData->RecentPeriod = pTargetData->PositionReportTicks - last_report_ticks;
        ...
        else //No data within last 30 minutes
        {
            have_updated_data = false;
            wxThread::Sleep(1000);
        }
    }
    /*if (checkHistory&&!checkHistoryDone)
     {
     checkHistory = false;
     checkHistoryDone = true;
     cc1->Enable();
     wxString msg(_("Ship retrieval complete."));
     md = new wxMessageDialog(cc1, msg, _("Siyara Message"), wxICON_INFORMATION | wxCENTRE);
     md->ShowModal();
     }*/
}
