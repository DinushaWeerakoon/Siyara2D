...
int AW_counter = 0;
int AWmmsi1, AWmmsi2;
double AWship1Lat, AWship1Lon, AWship2Lat, AWship2Lon;
//-------------------------------
...
void AISTargetListDialog::CreateControls()
{
    ...
    m_pListCtrlAISTargets->InsertColumn(tlIMO, _("IMO"), wxLIST_FORMAT_LEFT, width);
    s_width = tkz.GetNextToken();
    ...
    /*
     Added by Randika Ariyarathna
     */
    m_pButtonAnchorWatch = new wxButton(winr, wxID_ANY, _("Set Anchor Watch"), wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW);
    m_pButtonAnchorWatch->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(AISTargetListDialog::SetAnchorWatch), NULL, this);
    bsRouteButtonsInner->Add( m_pButtonAnchorWatch, 0, wxEXPAND | wxALL, 0 );
    //-anchorwatch------
...
void AISTargetListDialog::UpdateButtons()
{
     ...
     */
    if (AW_counter > 1)
        m_pButtonAnchorWatch->Enable(false);
    else
        m_pButtonAnchorWatch->Enable(enable);
    //-anchorwatch----------
}
...
/*
 Added by Randika Ariyarathna
 */
void AISTargetListDialog::SetAnchorWatch(wxCommandEvent& event)
{
    long selItemID = -1;
    ...
    if (m_pdecoder)
    {
        AIS_Target_Data *pAISTarget = m_pdecoder->Get_Target_Data_From_MMSI(m_pMMSI_array->Item(selItemID));
        if (pAISTarget)
        {
            int chk_mmsi = pAISTarget->MMSI;
            
            if (AW_counter == 0) //ready for first
            {
                double zlat = pAISTarget->Lat;
                double zlon = pAISTarget->Lon;
            ...
            else if (AW_counter == 1 && chk_mmsi != AWmmsi1) //ready for second
            {
                double zlat = pAISTarget->Lat;
                double zlon = pAISTarget->Lon;
                ...
                if (pAnchorWatchPoint2 == NULL)
                {
                    pAnchorWatchPoint2 = pWP;
    ...
    if( m_pdecoder ) {
        ...
        for( it = ( *current_targets ).begin(); it != ( *current_targets ).end(); ++it, ++index ) {
            ...
            if( NULL != pAISTarget ) {
                ...
                if ((pAISTarget->MMSI / 10000000 != 99) && (pAISTarget->b_positionOnceValid) && (pAISTarget->Range_NM <= g_AisTargetList_range))
                    b_add = true;
                //else if (!pAISTarget->b_positionOnceValid)
                else if ((pAISTarget->MMSI / 10000000 != 99) && !pAISTarget->b_positionOnceValid)
                    b_add = true;
                //-buoys--------
                if(b_add){
