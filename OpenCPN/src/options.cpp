...
/**
 ...
 These variable are for history related functions
 */
...
//-wrc----------------
/*
 Added by Randika Ariyarathna
 Modified by Chamal Perera
 */
 EVT_BUTTON(ID_RESET_QUERY, options::OnResetButtonClick)
 EVT_BUTTON(ID_HISTORY_QUERY, options::OnHistoryButtonClick)
 //-wrc----------------
...
void options::CreateControls()
{
	 ...
	 */
	// Create Filter Tab
	m_pageFilter = CreatePanel(_("Search"));
	AISFilterPanel = AddPage(m_pageFilter, _("Search"));
	wxBoxSizer* AISFilterPanelSizer = new wxBoxSizer(wxVERTICAL);
	AISFilterPanel->SetSizer(AISFilterPanelSizer);

	/*
	 wxStaticBox* StaticBoxAisQuery = new wxStaticBox(AISFilterPanel, wxID_ANY, _("Currently executing AIS_DB query"));
	 wxStaticBoxSizer* StaticBoxSizerAisQuery = new wxStaticBoxSizer(StaticBoxAisQuery, wxVERTICAL);
	 AISFilterPanelSizer->Add(StaticBoxSizerAisQuery, 0, wxEXPAND|wxALL, border_size);

	 MainQueryBox = new wxTextCtrl(AISFilterPanel, wxID_ANY, AIS_Query, wxDefaultPosition, wxSize(wxALL,80),wxTE_MULTILINE | wxTE_RICH , wxDefaultValidator, wxTextCtrlNameStr);
	 MainQueryBox->SetEditable(false);
	 StaticBoxSizerAisQuery->Add(MainQueryBox,0, wxEXPAND|wxALL, border_size);

	 //Query reset button
	 wxButton* resetButton = new wxButton( AISFilterPanel, ID_RESET_QUERY, _("Reset Query") );
	 StaticBoxSizerAisQuery->Add(resetButton, 0, wxALIGN_CENTER_HORIZONTAL|wxALL, 2);
	 */

	//Add selections
	wxStaticBox* SelectByBox = new wxStaticBox(AISFilterPanel, wxID_ANY, _("Search By"));
	wxStaticBoxSizer* SelectByBoxSizer = new wxStaticBoxSizer(SelectByBox, wxVERTICAL);
	AISFilterPanelSizer->Add(SelectByBoxSizer, 0, wxEXPAND | wxALL, border_size);

	wxFlexGridSizer *SelectByGrid = new wxFlexGridSizer(2);
	SelectByGrid->AddGrowableCol(1);
	SelectByBoxSizer->Add(SelectByGrid, 0, wxALL | wxEXPAND, border_size);

	//------Name-------
	wxStaticText *Name_Dataset = new wxStaticText(AISFilterPanel, wxID_ANY, _("    A Vessel Name"));
	SelectByGrid->Add(Name_Dataset, 1, wxALIGN_LEFT, group_item_spacing);
	NameDataset = new wxChoice(AISFilterPanel, wxID_ANY, wxDefaultPosition, wxSize(230, -1));
	SelectByGrid->Add(NameDataset, 1, wxALIGN_RIGHT, group_item_spacing);

	//-------Spacer-------
	SelectByGrid->Add(5, 8, 0, wxALL, 5); SelectByGrid->Add(5, 8, 0, wxALL, 5);

	//------Call-------
	wxStaticText *Call_Dataset = new wxStaticText(AISFilterPanel, wxID_ANY, _("    A Call Sign"));
	SelectByGrid->Add(Call_Dataset, 1, wxALIGN_LEFT, group_item_spacing);
	CallDataset = new wxChoice(AISFilterPanel, wxID_ANY, wxDefaultPosition, wxSize(230, -1));
	SelectByGrid->Add(CallDataset, 1, wxALIGN_RIGHT, group_item_spacing);

	//-------Spacer-------
	SelectByGrid->Add(5, 8, 0, wxALL, 5); SelectByGrid->Add(5, 8, 0, wxALL, 5);

	//------MMSI-------
	wxStaticText *MMSI_Dataset = new wxStaticText(AISFilterPanel, wxID_ANY, _("    A MMSI Number"));
	SelectByGrid->Add(MMSI_Dataset, 1, wxALIGN_LEFT, group_item_spacing);
	MMSIDataset = new wxChoice(AISFilterPanel, wxID_ANY, wxDefaultPosition, wxSize(230, -1));
	SelectByGrid->Add(MMSIDataset, 1, wxALIGN_RIGHT, group_item_spacing);

	//-------Spacer-------
	SelectByGrid->Add(5, 8, 0, wxALL, 5); SelectByGrid->Add(5, 8, 0, wxALL, 5);

	//------IMO-------
	wxStaticText *IMO_Dataset = new wxStaticText(AISFilterPanel, wxID_ANY, _("    An IMO Number"));
	SelectByGrid->Add(IMO_Dataset, 1, wxALIGN_LEFT, group_item_spacing);
	IMODataset = new wxChoice(AISFilterPanel, wxID_ANY, wxDefaultPosition, wxSize(230, -1));
	SelectByGrid->Add(IMODataset, 1, wxALIGN_RIGHT, group_item_spacing);

	//-------Spacer-------
	SelectByGrid->Add(5, 8, 0, wxALL, 5); SelectByGrid->Add(5, 8, 0, wxALL, 5);

	//------Type-------
	wxStaticText *Type_Dataset = new wxStaticText(AISFilterPanel, wxID_ANY, _("    A Ship Type"));
	SelectByGrid->Add(Type_Dataset, 1, wxALIGN_LEFT, group_item_spacing);
	TypeDataset = new wxChoice(AISFilterPanel, wxID_ANY, wxDefaultPosition, wxSize(230, -1));
	SelectByGrid->Add(TypeDataset, 1, wxALIGN_RIGHT, group_item_spacing);

	//-------Spacer-------
	SelectByGrid->Add(5, 8, 0, wxALL, 5); SelectByGrid->Add(5, 8, 0, wxALL, 5);

	//------Navigation Status-------
	wxStaticText *NavStatus_Dataset = new wxStaticText(AISFilterPanel, wxID_ANY, _("    A Ship Navigation Status"));
	SelectByGrid->Add(NavStatus_Dataset, 1, wxALIGN_LEFT, group_item_spacing);
	NavStatusDataset = new wxChoice(AISFilterPanel, wxID_ANY, wxDefaultPosition, wxSize(230, -1));
	SelectByGrid->Add(NavStatusDataset, 1, wxALIGN_RIGHT, group_item_spacing);

	//-new----------------------
	wxStaticBox* StaticBoxAisQuery = new wxStaticBox(AISFilterPanel, wxID_ANY, _(""));
	wxStaticBoxSizer* StaticBoxSizerAisQuery = new wxStaticBoxSizer(StaticBoxAisQuery, wxVERTICAL);
	AISFilterPanelSizer->Add(StaticBoxSizerAisQuery, 0, wxEXPAND | wxALL, border_size);

	StaticBoxAisQuery->Hide();
	...
	//Query reset button
	wxButton* resetButton = new wxButton(AISFilterPanel, ID_RESET_QUERY, _("View all ships"));
	//StaticBoxSizerAisQuery->Add(resetButton, 0, wxALIGN_CENTER_HORIZONTAL, 2);
	SelectByBoxSizer->Add(resetButton, 0, wxALIGN_CENTER_HORIZONTAL, 2);
	//-new----------------------

	//----------------------------

	if (connectedToDB && have_updated_data)
	{
		MYSQL *connection_2;
		connection_2 = mysql_init(NULL);

		if (mysql_real_connect(connection_2, connected_db_ip.mb_str(), "ais_data_viewer", "password", "cop_db", 3306, NULL, 0) != 0)
		{
			NameDataset->Append(_("None"));
			MYSQL_RES *res_set_1;
			MYSQL_ROW row_1;
			std::string query_1 = "select ship_name from decoded_ais_data where mmsi < 994170000 and last_received_date = curdate() and last_received_time > (curtime() - 003000) order by ship_name";
			if (!mysql_query(connection_2, query_1.c_str()))
			{
				res_set_1 = mysql_store_result(connection_2);

				while ((row_1 = mysql_fetch_row(res_set_1)) != NULL)
				{
					wxString wxName(row_1[0], wxConvUTF8);
					if (wxName != _(""))
						NameDataset->Append(wxName.Trim());
				}
			}

			CallDataset->Append(_("None"));
			MYSQL_RES *res_set_2;
			MYSQL_ROW row_2;
			std::string query_2 = "select callsign from decoded_ais_data where mmsi < 994170000 and last_received_date = curdate() and last_received_time > (curtime() - 003000) order by callsign";
			if (!mysql_query(connection_2, query_2.c_str()))
			{
				res_set_2 = mysql_store_result(connection_2);

				while ((row_2 = mysql_fetch_row(res_set_2)) != NULL)
				{
					wxString wxCall(row_2[0], wxConvUTF8);
					if (wxCall != _(""))
						CallDataset->Append(wxCall.Trim());
				}
			}

			MMSIDataset->Append(_("None"));
			MYSQL_RES *res_set_3;
			MYSQL_ROW row_3;
			std::string query_3 = "select mmsi from decoded_ais_data where mmsi < 994170000 and last_received_date = curdate() and last_received_time > (curtime() - 003000) order by mmsi";
			if (!mysql_query(connection_2, query_3.c_str()))
			{
				res_set_3 = mysql_store_result(connection_2);

				while ((row_3 = mysql_fetch_row(res_set_3)) != NULL)
				{
					if (row_3[0] != NULL)
					{
						int mmsi = atoi(row_3[0]);
						wxString wxMMSI;
						wxMMSI << mmsi;
						MMSIDataset->Append(wxMMSI);
					}
				}
			}

			IMODataset->Append(_("None"));
			MYSQL_RES *res_set_4;
			MYSQL_ROW row_4;
			std::string query_4 = "select imo_number from decoded_ais_data where mmsi < 994170000 and last_received_date = curdate() and last_received_time > (curtime() - 003000) order by imo_number";
			if (!mysql_query(connection_2, query_4.c_str()))
			{
				res_set_4 = mysql_store_result(connection_2);

				while ((row_4 = mysql_fetch_row(res_set_4)) != NULL)
				{
					if (row_4[0] != NULL)
					{
						int imo = atoi(row_4[0]);
						wxString wxIMO;
						wxIMO << imo;
						IMODataset->Append(wxIMO);
					}
				}
			}

			TypeDataset->Append(_("None"));
			MYSQL_RES *res_set_5;
			MYSQL_ROW row_5;
			std::string query_5 = "select type_of_ship_and_cargo from decoded_ais_data where mmsi < 994170000 and last_received_date = curdate() and last_received_time > (curtime() - 003000) group by type_of_ship_and_cargo order by type_of_ship_and_cargo";
			if (!mysql_query(connection_2, query_5.c_str()))
			{
				res_set_5 = mysql_store_result(connection_2);

				while ((row_5 = mysql_fetch_row(res_set_5)) != NULL)
				{
					if (row_5[0] != NULL)
					{
						int type = atoi(row_5[0]);
						wxString wxType;

						switch (type)
						{
						case 30: wxType = _("Vessel Fishing"); break;
						case 31: wxType = _("Vessel Towing"); break;
						case 32: wxType = _("Vessel Towing, Long"); break;
						case 33: wxType = _("Vessel Dredging"); break;
						case 34: wxType = _("Vessel Diving"); break;
						case 35: wxType = _("Military Vessel"); break;
						case 36: wxType = _("Sailing Vessel"); break;
						case 37: wxType = _("Pleasure craft"); break;
						case 50: wxType = _("Pilot Vessel"); break;
						case 51: wxType = _("Search and Rescue Vessel"); break;
						case 52: wxType = _("Tug"); break;
						case 53: wxType = _("Port Tender"); break;
						case 54: wxType = _("Pollution Control Vessel"); break;
						case 55: wxType = _("Law Enforcement Vessel"); break;
						case 58: wxType = _("Medical Transport"); break;
						default: wxType = _("Unknown"); break;
						}
						if ((type >= 40) && (type < 50)) wxType = _("High Speed Craft");
						else if ((type >= 60) && (type < 70)) wxType = _("Passenger Ship");
						else if ((type >= 70) && (type < 80)) wxType = _("Cargo Ship");
						else if ((type >= 80) && (type < 90)) wxType = _("Tanker");

						if (TypeDataset->FindString(wxType) == -1)
							TypeDataset->Append(wxType);
					}
				}
			}

			NavStatusDataset->Append(_("None"));
			MYSQL_RES *res_set_6;
			MYSQL_ROW row_6;
			std::string query_6 = "select navigational_status from decoded_ais_data where mmsi < 994170000 and last_received_date = curdate() and last_received_time > (curtime() - 003000) group by navigational_status order by navigational_status";
			if (!mysql_query(connection_2, query_6.c_str()))
			{
				res_set_6 = mysql_store_result(connection_2);

				while ((row_6 = mysql_fetch_row(res_set_6)) != NULL)
				{
					if (row_6[0] != NULL)
					{
						int navstatus = atoi(row_6[0]);
						wxString wxNavStatus;

						switch (navstatus)
						{
						case 0: wxNavStatus = _("Underway"); break;
						case 1: wxNavStatus = _("At Anchor"); break;
						case 2: wxNavStatus = _("Not Under Command"); break;
						case 3: wxNavStatus = _("Restricted Manoeuvrability"); break;
						case 4: wxNavStatus = _("Constrained by draught"); break;
						case 5: wxNavStatus = _("Moored"); break;
						case 6: wxNavStatus = _("Aground"); break;
						case 7: wxNavStatus = _("Engaged in Fishing"); break;
						case 8: wxNavStatus = _("Under way sailing"); break;
						default: wxNavStatus = _("Undefined"); break;
						}

						NavStatusDataset->Append(wxNavStatus);
					}
				}
			}
		}
	...
	NameDataset->Select(0);
	CallDataset->Select(0);
	MMSIDataset->Select(0);
	IMODataset->Select(0);
	TypeDataset->Select(0);
	NavStatusDataset->Select(0);

	//-History--------------------

	/**
	 Created by Chamal Perera
	 This section is to implement the history related functions
	 */

	m_pageFilter = CreatePanel(_("History"));
	HistoryPanel = AddPage(m_pageFilter, _("History"));
	wxBoxSizer* HistoryPanelSizer = new wxBoxSizer(wxVERTICAL);
	HistoryPanel->SetSizer(HistoryPanelSizer);

	wxStaticBox* HistoryBox = new wxStaticBox(HistoryPanel, wxID_ANY, _("View History"));
	wxStaticBoxSizer* HistoryBoxSizer = new wxStaticBoxSizer(HistoryBox, wxVERTICAL);
	HistoryPanelSizer->Add(HistoryBoxSizer, 0, wxEXPAND | wxALL, border_size);

	wxFlexGridSizer *HisoryGrid = new wxFlexGridSizer(2);
	HisoryGrid->AddGrowableCol(1);
	HistoryBoxSizer->Add(HisoryGrid, 0, wxALL | wxEXPAND, border_size);

	wxStaticText *historyOn = new wxStaticText(HistoryPanel, wxID_ANY, _("Date : "));
	HisoryGrid->Add(historyOn, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);

	datePicker = new wxDatePickerCtrl(HistoryPanel, wxID_ANY, wxDefaultDateTime);
	HisoryGrid->Add(datePicker, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);

	//-------Spacer-------
	HisoryGrid->Add(5, 8, 0, wxALL, 5); HisoryGrid->Add(5, 8, 0, wxALL, 5);

	wxStaticText *From = new wxStaticText(HistoryPanel, wxID_ANY, _("Time : "));
	HisoryGrid->Add(From, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);

	timePicker = new wxTimePickerCtrl(HistoryPanel, wxID_ANY, wxDefaultDateTime);
	HisoryGrid->Add(timePicker, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);

	//-------Spacer-------
	HisoryGrid->Add(5, 8, 0, wxALL, 5); HisoryGrid->Add(5, 8, 0, wxALL, 5);
	wxButton* historyButton = new wxButton(HistoryPanel, ID_HISTORY_QUERY, _("View "));
	HistoryBoxSizer->Add(historyButton, 0, wxALIGN_CENTER_HORIZONTAL, 2);

	//Query reset button
	wxButton* resetButtonHistory = new wxButton(HistoryPanel, ID_RESET_QUERY, _("View all ships"));
...
void options::OnApplyClick(wxCommandEvent& event)
{
	...
	if (NameDataset->GetSelection() != 0)
	{
		AIS_Query = _("select * from decoded_ais_data where (ship_name = \'");
		AIS_Query.append(NameDataset->GetStringSelection());
		AIS_Query.append(_("\' or mmsi > 994170000) and last_received_date = curdate() and last_received_time > (curtime() - 003000)"));
	}

	else if (CallDataset->GetSelection() != 0)
	{
		AIS_Query = _("select * from decoded_ais_data where (callsign = \'");
		AIS_Query.append(CallDataset->GetStringSelection());
		AIS_Query.append(_("\' or mmsi > 994170000) and last_received_date = curdate() and last_received_time > (curtime() - 003000)"));
	}

	else if (MMSIDataset->GetSelection() != 0)
	{
		AIS_Query = _("select * from decoded_ais_data where (mmsi = ");
		AIS_Query.append(MMSIDataset->GetStringSelection());
		AIS_Query.append(_(" or mmsi > 994170000) and last_received_date = curdate() and last_received_time > (curtime() - 003000)"));
	}

	else if (IMODataset->GetSelection() != 0)
	{
		AIS_Query = _("select * from decoded_ais_data where (imo_number = ");
		AIS_Query.append(IMODataset->GetStringSelection());
		AIS_Query.append(_(" or  mmsi > 994170000) and last_received_date = curdate() and last_received_time > (curtime() - 003000)"));
	}

	else if (TypeDataset->GetSelection() != 0)
	{
		AIS_Query = _("select * from decoded_ais_data where (type_of_ship_and_cargo ");
		wxString selectedShipType = TypeDataset->GetStringSelection();

		if (selectedShipType == _("Vessel Fishing")) AIS_Query.append(_("= 30"));
		else if (selectedShipType == _("Vessel Towing")) AIS_Query.append(_("= 31"));
		else if (selectedShipType == _("Vessel Towing, Long")) AIS_Query.append(_("= 32"));
		else if (selectedShipType == _("Vessel Dredging")) AIS_Query.append(_("= 33"));
		else if (selectedShipType == _("Vessel Diving")) AIS_Query.append(_("= 34"));
		else if (selectedShipType == _("Military Vessel")) AIS_Query.append(_("= 35"));
		else if (selectedShipType == _("Sailing Vessel")) AIS_Query.append(_("= 36"));
		else if (selectedShipType == _("Pleasure craft")) AIS_Query.append(_("= 37"));
		else if (selectedShipType == _("Pilot Vessel")) AIS_Query.append(_("= 50"));
		else if (selectedShipType == _("Search and Rescue Vessel")) AIS_Query.append(_("= 51"));
		else if (selectedShipType == _("Tug")) AIS_Query.append(_("= 52"));
		else if (selectedShipType == _("Port Tender")) AIS_Query.append(_("= 53"));
		else if (selectedShipType == _("Pollution Control Vessel")) AIS_Query.append(_("= 54"));
		else if (selectedShipType == _("Law Enforcement Vessel")) AIS_Query.append(_("= 55"));
		else if (selectedShipType == _("Medical Transport")) AIS_Query.append(_("= 58"));
		else if (selectedShipType == _("High Speed Craft")) AIS_Query.append(_(">= 40 and type_of_ship_and_cargo < 50"));
		else if (selectedShipType == _("Passenger Ship")) AIS_Query.append(_(">= 60 and type_of_ship_and_cargo < 70"));
		else if (selectedShipType == _("Cargo Ship")) AIS_Query.append(_(">= 70 and type_of_ship_and_cargo < 80"));
		else if (selectedShipType == _("Tanker")) AIS_Query.append(_(" >= 80 and type_of_ship_and_cargo < 90"));
		else if (selectedShipType == _("Unknown")) AIS_Query.append(_("< 30 or type_of_ship_and_cargo >= 90"));

		AIS_Query.append(_(" or  mmsi > 994170000) and last_received_date = curdate() and last_received_time > (curtime() - 003000)"));
	}

	else if (NavStatusDataset->GetSelection() != 0)
	{
		AIS_Query = _("select * from decoded_ais_data where (navigational_status ");
		wxString selectedNavStatus = NavStatusDataset->GetStringSelection();

		if (selectedNavStatus == _("Underway")) AIS_Query.append(_("= 0"));
		else if (selectedNavStatus == _("At Anchor")) AIS_Query.append(_("= 1"));
		else if (selectedNavStatus == _("Not Under Command")) AIS_Query.append(_("= 2"));
		else if (selectedNavStatus == _("Restricted Manoeuvrability")) AIS_Query.append(_("= 3"));
		else if (selectedNavStatus == _("Constrained by draught")) AIS_Query.append(_("= 4"));
		else if (selectedNavStatus == _("Moored")) AIS_Query.append(_("= 5"));
		else if (selectedNavStatus == _("Aground")) AIS_Query.append(_("= 6"));
		else if (selectedNavStatus == _("Engaged in Fishing"))AIS_Query.append(_("= 7"));
		else if (selectedNavStatus == _("Under way sailing"))AIS_Query.append(_("= 8"));
		else if (selectedNavStatus == _("Undefined"))AIS_Query.append(_("> 8"));
...
void options::OnResetButtonClick(wxCommandEvent& event)
{
	if (AIS_Query != _("select * from decoded_ais_data where last_received_date = curdate() and last_received_time > (curtime() - 003000)"))
	{
		AIS_Query = _("select * from decoded_ais_data where last_received_date = curdate() and last_received_time > (curtime() - 003000)");
		MainQueryBox->SetValue(AIS_Query);
	}
}
//-wrc-------------------------
...
/**
 Modified by Chamal Perera
 This will show a snapshot of the situation of harbour on a given time.
 Triggers when history view button is clicked.
 */

void options::OnHistoryButtonClick(wxCommandEvent& event)
{
	wxDateTime time = timePicker->GetValue();
	/*wxString times = time.FormatISODate();
	std::string dateStr = std::string(times.mb_str());*/
	//const wxChar* constTime = timeS.c_str();
	//wxDateTime dateTime = wxDateTime::Now();

	//dateTime.ParseTime(constTime);

	const wxTimeSpan *diffTimeL = new wxTimeSpan(0, 15, 0, 0);
	wxString minLb = time.Subtract(*diffTimeL).FormatISOTime();
	wxLogMessage(minLb);

	const wxTimeSpan *diffTimeU = new wxTimeSpan(0, 30, 0, 0);
	wxString minUb = time.Add(*diffTimeU).FormatISOTime();
	wxLogMessage(minUb);

	std::string timeLb = std::string(minLb.mb_str());
	std::string timeUb = std::string(minUb.mb_str());

	wxDateTime date = datePicker->GetValue();
	wxString dates = date.FormatISODate();
	std::string dateStr = std::string(dates.mb_str());

	wxCommandEvent ev(wxEVT_COMMAND_BUTTON_CLICKED, xID_OK);
	m_OKButton->GetEventHandler()->ProcessEvent(ev);
	//EndModal(xID_OK);
	std::string qry = "SELECT d.mmsi, t.navigational_status, t.rate_of_turn, t.speed_over_ground, t.longitude, t.latitude, t.course_over_ground, t.true_heading,  t.last_received_date, t.last_received_time, d.sync_state, d.slot_timeout, d.ais_version, d.imo_number, d.callsign, d.ship_name, d.type_of_ship_and_cargo, d.distance_from_bow, d.distance_from_stern, d.distance_from_port, d.distance_from_starboard, d.estimated_time_of_arrival, d.maximum_present_static_draught,d.ship_destination,t.ais_class FROM decoded_ais_data d INNER JOIN(SELECT mmsi,last_received_date,true_heading, latitude, longitude, speed_over_ground, course_over_ground, navigational_status, rate_of_turn, ais_class, last_received_time ,CONCAT(last_received_date,' ' , last_received_time) AS lDT FROM decoded_ais_tracking ORDER BY lDT DESC) t ON t.mmsi=d.mmsi WHERE t.lDT BETWEEN '" + dateStr + " " + timeLb + "' AND '" + dateStr + " " + timeUb + "' GROUP BY t.mmsi";

	wxString qryStr(qry.c_str(), wxConvUTF8);
